...
def get_available_dates(n_days: int = 7) -> list[str]:
    """Return the last n_days dates (GIBS has ~3-5 hour latency for daily products)."""
    today = datetime.now(timezone.utc)
    # MODIS daily products typically available by ~14:00 UTC
    if today.hour < 14:
...
def fetch_sequence(
    ...
    Pads with last good frame if any fetch fails.
    """
    dates = get_available_dates(n_days=n_frames + 3)
    ...
    while len(images) < n_frames and images:
        ...
        fetched_dates.append(fetched_dates[-1] + " (padded)")
